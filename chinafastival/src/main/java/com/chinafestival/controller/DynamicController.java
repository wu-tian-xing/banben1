...
public class DynamicController {
    ...
    public JSONObject qureyUserDynamic(@RequestParam("page")int page, @RequestParam("limit")int limit,@RequestParam("user_id")String user_id){
        JSONObject json = new JSONObject();
        int page1 = page-1;
        int page2 = page1 * limit;
        json.put("code", 0);
    ...
    public DynamicView selectOneDyname(int d_id){
        return dynamicService.selectOneDyname(d_id);
    }
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "Keyword",value = "关键字(details,u_id)",dataType = "String",required = true),
    })
