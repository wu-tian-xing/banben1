...
public interface DynamicMapper {
    ...
    //查询个人所有动态
    List<DynamicView> qureyUserDynamic(@Param("page")int page, @Param("limit")int limit,@Param("user_id")String user_id);

    //查看一个动态所有信息
    DynamicView  selectOneDyname(@Param("d_id")int d_id);

    //评论数
    int updateNumReview(@Param("num_review")int num_review,@Param("d_id")int d_id);

    //模糊查询
    List<DynamicView> selectDynamic(@Param("page") int page, @Param("limit") int limit, @Param("Keyword")String Keyword);
